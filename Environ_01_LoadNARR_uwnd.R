# Load NARR uwnd data into pep DB
# Created by J. London
...
wd <- "O://Data/GIS_External/Climate/NARR/uwnd/"
setwd(wd)
fls <- dir(pattern = '*.nc')
...
for(i in unique(yrs)){
  dbGetQuery(con, paste("DROP TABLE IF EXISTS environ.tbl_narr_uwnd_", i, sep = ""))
  sql <- paste("CREATE TABLE environ.tbl_narr_uwnd_", i, "(CHECK (lower(fdatetime_range) >= TIMESTAMP WITH TIME ZONE '", i, 
               "-01-01 00:00:00-00' AND upper(fdatetime_range) <= TIMESTAMP WITH TIME ZONE'", i, 
               "-12-31 23:59:59-00' )) INHERITS (environ.tbl_narr_uwnd);", sep = "")
  dbGetQuery(con, sql)
  rm(sql)
  ...
  for (b in c(1:x)){
    ...
    sql <- paste("ALTER TABLE environ.temp_uwnd ADD COLUMN fdatetime_range tstzrange; UPDATE environ.temp_uwnd SET fdatetime_range = tstzrange('", format(band_dt, "%Y-%m-%d %H:%M:%S"), "-00','", format(band_dt_end, "%Y-%m-%d %H:%M:%S"), "-00','[]');", sep="")
    dbGetQuery(con, sql)
    sql <- paste("INSERT INTO environ.tbl_narr_uwnd_", i, " (rid, fdatetime_range, rast) SELECT rid, fdatetime_range, rast from environ.temp_uwnd;", sep = "")
    dbGetQuery(con, sql)
  }
  sql <- paste("CREATE INDEX idx_narr_uwnd_fdatetime_range_", i, " ON environ.tbl_narr_uwnd_", i, " USING GIST (fdatetime_range);", sep = "")
  dbGetQuery(con, sql)
}

# Delete temp tables from DB
dbGetQuery(con, "DROP TABLE IF EXISTS environ.temp_uwnd")

# Create index on layer (if it does not exist)
# dbSendQuery(con, "CREATE INDEX tbl_narr_uwnd_rast_idx ON environ.tbl_narr_uwnd USING gist(ST_ConvexHull(rast))")
